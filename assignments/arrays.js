...
let myCar = inventory[32];
console.log(`Car 33 is a ${myCar["car_year"]} ${myCar["car_make"]} ${myCar["car_model"]} .` );

// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = inventory[49];
console.log(`Car make: ${lastCar["car_make"]}, Car model: ${lastCar["car_model"]}.`);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
let carModels = [];

function forLoopModels(){
    for(let i = 0; i < inventory.length; i++){
        let aCar = inventory[i]
        carModels.push(aCar["car_model"])
    }
}
forLoopModels()
...
let carYears = [];
function forLoopYears(){
    for(let i = 0; i < inventory.length; i++){
        let aCar = inventory[i]
        carYears.push(aCar["car_year"])
    }
}
forLoopYears()
...
let oldCars = [];
function forLoopOldYears(){
    for(let i = 0; i < inventory.length; i++){
        let aCar = inventory[i]
        if(aCar["car_year"] < 2000){
           oldCars.push(aCar["car_year"]) 
        }
...
let BMWAndAudi = [];
function forLoopSpecific(){
    for(let i = 0; i < inventory.length; i++){
        let aCar = inventory[i]
        if(aCar["car_make"] === "Audi" || aCar["car_make"] === "BMW"){
           BMWAndAudi.push(aCar) 
        }
